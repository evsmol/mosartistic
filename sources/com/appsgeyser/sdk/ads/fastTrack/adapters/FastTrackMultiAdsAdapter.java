...
public class FastTrackMultiAdsAdapter extends FastTrackBaseAdapter {
    ...
    public void init() {
        ...
        if (arrayList.contains("appodealSdk")) {
            ...
            String str = null;
            if (!(this.fastTrackSdkModel.getFullscreenPlacementIdMap() == null || this.fastTrackSdkModel.getFullscreenPlacementIdMap().get("appodealSdk") == null)) {
                str = this.fastTrackSdkModel.getFullscreenPlacementIdMap().get("appodealSdk").getAppId();
            }
            if (str == null || str.isEmpty()) {
                str = this.fastTrackSdkModel.getBannerPlacementIdMap().get("appodealSdk").getAppId();
            }
            Appodeal.initialize((Activity) this.context, str, 7, (ApdInitializationCallback) new ApdInitializationCallback() {
    ...
    public void showFullscreen(String str, String str2, boolean z, final Context context) {
        if (!z || System.currentTimeMillis() - this.preferencesCoder.getPrefLong("appsgeyserSdk_lastRequestTiming", 0) > ((long) getFullscreenFrequencyTimerValue(str2).intValue())) {
            ...
            android.util.Log.d("multiAdsTag", "smartAds fullscreen attempt to show canceled due to intensity settings");
            if (this.fullscreenListener != null) {
