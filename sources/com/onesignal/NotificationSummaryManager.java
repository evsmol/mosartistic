...
class NotificationSummaryManager {
    static void updatePossibleDependentSummaryOnDismiss(Context context, OneSignalDb oneSignalDb, int i) {
        Cursor query = oneSignalDb.query("notification", new String[]{"group_id"}, "android_notification_id = " + i, (String[]) null, (String) null, (String) null, (String) null);
    ...
    private static Cursor internalUpdateSummaryNotificationAfterChildRemoved(Context context, OneSignalDb oneSignalDb, String str, boolean z) {
        Cursor query = oneSignalDb.query("notification", new String[]{"android_notification_id", "created_time", "full_data"}, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 0", new String[]{str}, (String) null, (String) null, "_id DESC");
        ...
        if (count == 0 && !str.equals(OneSignalNotificationManager.getGrouplessSummaryKey())) {
            ...
            oneSignalDb.update("notification", contentValues, "android_notification_id = " + summaryNotificationId, (String[]) null);
            return query;
        } else if (count == 1) {
            query.close();
    ...
    private static void restoreSummary(Context context, String str) {
        ...
        Cursor cursor = null;
        try {
            cursor = OneSignalDbHelper.getInstance(context).query("notification", OSNotificationRestoreWorkManager.COLUMNS_FOR_RESTORE, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 0", strArr, (String) null, (String) null, (String) null);
