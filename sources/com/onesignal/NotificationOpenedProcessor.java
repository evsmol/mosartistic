...
class NotificationOpenedProcessor {
    ...
    private static void addChildNotifications(JSONArray jSONArray, String str, OneSignalDbHelper oneSignalDbHelper) {
        OneSignalDbHelper oneSignalDbHelper2 = oneSignalDbHelper;
        Cursor query = oneSignalDbHelper2.query("notification", new String[]{"full_data"}, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 0", new String[]{str}, (String) null, (String) null, (String) null);
        if (query.getCount() > 1) {
            query.moveToFirst();
            do {
                try {
    ...
    private static void markNotificationsConsumed(Context context, Intent intent, OneSignalDbHelper oneSignalDbHelper, boolean z) {
        ...
        oneSignalDbHelper.update("notification", newContentValuesWithConsumed(intent), str, strArr);
