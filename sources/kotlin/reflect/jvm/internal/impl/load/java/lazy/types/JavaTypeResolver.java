...
public final class JavaTypeResolver {
    ...
    /* renamed from: c */
    private final LazyJavaResolverContext f12940c;
    ...
    public JavaTypeResolver(LazyJavaResolverContext lazyJavaResolverContext, TypeParameterResolver typeParameterResolver2) {
        ...
        this.f12940c = lazyJavaResolverContext;
    ...
    public final KotlinType transformJavaType(JavaType javaType, JavaTypeAttributes javaTypeAttributes) {
        ...
        if (javaType instanceof JavaPrimitiveType) {
            ...
            if (type != null) {
                simpleType = this.f12940c.getModule().getBuiltIns().getPrimitiveKotlinType(type);
            } else {
                simpleType = this.f12940c.getModule().getBuiltIns().getUnitType();
            }
            ...
            if (javaType instanceof JavaArrayType) {
                ...
                SimpleType defaultBound = this.f12940c.getModule().getBuiltIns().getDefaultBound();
                Intrinsics.checkNotNullExpressionValue(defaultBound, "c.module.builtIns.defaultBound");
                return defaultBound;
            } else if (javaType == null) {
                SimpleType defaultBound2 = this.f12940c.getModule().getBuiltIns().getDefaultBound();
    ...
    public final KotlinType transformArrayType(JavaArrayType javaArrayType, JavaTypeAttributes javaTypeAttributes, boolean z) {
        ...
        LazyJavaAnnotations lazyJavaAnnotations = new LazyJavaAnnotations(this.f12940c, javaArrayType, true);
        if (type != null) {
            SimpleType primitiveArrayKotlinType = this.f12940c.getModule().getBuiltIns().getPrimitiveArrayKotlinType(type);
        ...
        if (javaTypeAttributes.isForAnnotationParameter()) {
            SimpleType arrayType = this.f12940c.getModule().getBuiltIns().getArrayType(z ? Variance.OUT_VARIANCE : Variance.INVARIANT, transformJavaType, lazyJavaAnnotations);
        ...
        SimpleType arrayType2 = this.f12940c.getModule().getBuiltIns().getArrayType(Variance.INVARIANT, transformJavaType, annotations);
        Intrinsics.checkNotNullExpressionValue(arrayType2, "c.module.builtIns.getArr…mponentType, annotations)");
        return KotlinTypeFactory.flexibleType(arrayType2, this.f12940c.getModule().getBuiltIns().getArrayType(Variance.OUT_VARIANCE, transformJavaType, annotations).makeNullableAsSpecified(true));
    }
    ...
    private final SimpleType computeSimpleJavaClassifierType(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes, SimpleType simpleType) {
        Annotations lazyJavaAnnotations = simpleType == null ? new LazyJavaAnnotations(this.f12940c, javaClassifierType, false, 4, (DefaultConstructorMarker) null) : simpleType.getAnnotations();
        ...
        if (classifier instanceof JavaClass) {
            ...
            if (fqName != null) {
                ...
                if (mapKotlinClass == null) {
                    mapKotlinClass = this.f12940c.getComponents().getModuleClassResolver().resolveClass(javaClass);
                }
    ...
    private final TypeConstructor createNotFoundClass(JavaClassifierType javaClassifierType) {
        ...
        TypeConstructor typeConstructor = this.f12940c.getComponents().getDeserializedDescriptorResolver().getComponents().getNotFoundClasses().getClass(classId, CollectionsKt.listOf(0)).getTypeConstructor();
    ...
    private final ClassDescriptor mapKotlinClass(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes, FqName fqName) {
        if (javaTypeAttributes.isForAnnotationParameter() && Intrinsics.areEqual((Object) fqName, (Object) JavaTypeResolverKt.JAVA_LANG_CLASS_FQ_NAME)) {
            return this.f12940c.getComponents().getReflectionTypes().getKClass();
        }
        JavaToKotlinClassMapper javaToKotlinClassMapper = JavaToKotlinClassMapper.INSTANCE;
        ClassDescriptor mapJavaToKotlin$default = JavaToKotlinClassMapper.mapJavaToKotlin$default(javaToKotlinClassMapper, fqName, this.f12940c.getModule().getBuiltIns(), (Integer) null, 4, (Object) null);
    ...
    private final List<TypeProjection> computeRawTypeArguments(JavaClassifierType javaClassifierType, List<? extends TypeParameterDescriptor> list, TypeConstructor typeConstructor, JavaTypeAttributes javaTypeAttributes) {
        ...
        for (TypeParameterDescriptor typeParameterDescriptor : iterable) {
            if (TypeUtilsKt.hasTypeParameterRecursiveBounds(typeParameterDescriptor, (TypeConstructor) null, javaTypeAttributes.getVisitedTypeParameters())) {
                ...
                typeProjection = this.rawSubstitution.computeProjection(typeParameterDescriptor, javaClassifierType.isRaw() ? javaTypeAttributes : javaTypeAttributes.withFlexibility(JavaTypeFlexibility.INFLEXIBLE), new LazyWrappedType(this.f12940c.getStorageManager(), new JavaTypeResolver$computeRawTypeArguments$1$erasedUpperBound$1(this, typeParameterDescriptor, javaClassifierType, javaTypeAttributes, typeConstructor)));
            }
