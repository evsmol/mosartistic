...
public class RangedBeacon implements Serializable {
    ...
    public void commitMeasurements() {
        if (!getFilter().noMeasurementsAvailable()) {
            ...
            LogManager.m14725d("RangedBeacon", "calculated new runningAverageRssi: %s", Double.valueOf(calculateRssi));
        } else {
            LogManager.m14725d("RangedBeacon", "No measurements available to calculate running average", new Object[0]);
        }
    ...
    private RssiFilter getFilter() {
        if (this.mFilter == null) {
            try {
                ...
                LogManager.m14726e("RangedBeacon", "Could not construct RssiFilterImplClass %s", BeaconManager.getRssiFilterImplClass().getName());
            }
        }
        return this.mFilter;
    }
