...
public class BeaconParser implements Serializable {
    ...
    public BeaconParser setBeaconLayout(String str) {
        ...
        while (i < length) {
            ...
            if (z) {
                ...
                LogManager.m14725d("BeaconParser", "cannot parse term %s", str3);
        ...
        while (true) {
            ...
            if (pdu.getType() != 22 && pdu.getType() != -1) {
                if (LogManager.isVerboseLoggingEnabled()) {
                    LogManager.m14725d("BeaconParser", "Ignoring pdu type %02X", Byte.valueOf(pdu.getType()));
                }
            }
        }
        if (LogManager.isVerboseLoggingEnabled()) {
            LogManager.m14725d("BeaconParser", "Processing pdu type %02X: %s with startIndex: %d, endIndex: %d", Byte.valueOf(pdu.getType()), bytesToHex(bArr), Integer.valueOf(pdu.getStartIndex()), Integer.valueOf(pdu.getEndIndex()));
        }
        if (pdu == null) {
            if (LogManager.isVerboseLoggingEnabled()) {
                LogManager.m14725d("BeaconParser", "No PDUs to process in this packet.", new Object[0]);
            }
            beacon2 = beacon;
            z = true;
            i3 = 0;
            ...
            if (!z2) {
                if (getServiceUuid() == null) {
                    if (LogManager.isVerboseLoggingEnabled()) {
                        LogManager.m14725d("BeaconParser", "This is not a matching Beacon advertisement. (Was expecting %s.  The bytes I see are: %s", byteArrayToString(longToByteArray), bytesToHex(bArr));
                    }
                } else if (LogManager.isVerboseLoggingEnabled()) {
                    LogManager.m14725d("BeaconParser", "This is not a matching Beacon advertisement. Was expecting %s at offset %d and %s at offset %d.  The bytes I see are: %s", byteArrayToString(longToByteArray2), Integer.valueOf(this.mServiceUuidStartOffset.intValue() + i3), byteArrayToString(longToByteArray), Integer.valueOf(this.mMatchingBeaconTypeCodeStartOffset.intValue() + i3), bytesToHex(bArr));
                }
                beacon2 = null;
                z = true;
            } else {
                if (LogManager.isVerboseLoggingEnabled()) {
                    LogManager.m14725d("BeaconParser", "This is a recognized beacon advertisement -- %s seen", byteArrayToString(longToByteArray));
                    LogManager.m14725d("BeaconParser", "Bytes are: %s", bytesToHex(bArr));
                }
                beacon2 = beacon;
                z = false;
            }
            if (z2) {
                if (bArr2.length <= this.mLayoutSize.intValue() + i3 && this.mAllowPduOverflow.booleanValue()) {
                    if (LogManager.isVerboseLoggingEnabled()) {
                        LogManager.m14725d("BeaconParser", "Expanding buffer because it is too short to parse: " + bArr2.length + ", needed: " + (this.mLayoutSize.intValue() + i3), new Object[0]);
                    }
                ...
                while (true) {
                    ...
                    if (intValue > pdu.getEndIndex() && this.mIdentifierVariableLengthFlags.get(i4).booleanValue()) {
                        if (LogManager.isVerboseLoggingEnabled()) {
                            LogManager.m14725d("BeaconParser", "Need to truncate identifier by " + (intValue - pdu.getEndIndex()), new Object[0]);
                        }
                        ...
                        if (LogManager.isVerboseLoggingEnabled()) {
                            LogManager.m14725d("BeaconParser", "Cannot parse identifier " + i4 + " because PDU is too short.  endIndex: " + intValue + str3 + pdu.getEndIndex(), new Object[0]);
                        }
                        z = true;
                    }
                    i4++;
                ...
                while (i5 < this.mDataEndOffsets.size()) {
                    ...
                    if (intValue2 <= pdu.getEndIndex() || this.mAllowPduOverflow.booleanValue()) {
                        ...
                        if (LogManager.isVerboseLoggingEnabled()) {
                            LogManager.m14725d("BeaconParser", "Cannot parse data field " + i5 + " because PDU is too short.  endIndex: " + intValue2 + str3 + pdu.getEndIndex() + ".  Setting value to 0", new Object[i2]);
                        }
                        str4 = str3;
                ...
                if (this.mPowerStartOffset != null) {
                    ...
                    try {
                        if (intValue3 <= pdu.getEndIndex() || this.mAllowPduOverflow.booleanValue()) {
                            ...
                            try {
                                if (LogManager.isVerboseLoggingEnabled()) {
                                    LogManager.m14725d("BeaconParser", "Cannot parse power field because PDU is too short.  endIndex: " + intValue3 + str5 + pdu.getEndIndex(), new Object[0]);
                                }
    ...
    protected static String bytesToHex(byte[] bArr) {
        ...
        for (int i = 0; i < bArr.length; i++) {
            ...
            cArr[i2] = cArr2[b >>> 4];
            cArr[i2 + 1] = cArr2[b & Ascii.f15551SI];
        }
